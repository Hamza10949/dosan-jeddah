...
class PurchaseOrder(models.Model):
    ...
    total_amount_pays = fields.Integer(compute='_total_payment_count')

    @api.depends('partner_id')
    def _compute_payment_count(self):
        totalpay = self.env['account.payment'].search(
            [('ref', '=', self.name)])
...
# custom changes by nadia

    def _total_payment_count(self):
        ven_bills = self.env['account.payment'].search([])
        total = 0
        for record in ven_bills:
            for rec in self:
    ...
    def compute_amount_field(self):
        ...
        for record in ven_bills:
            for rec in self:
                ...
                # if rec.name==record.ref:
                if rec.cout_payment == 0:
                    return self.amount_total
                if rec.cout_payment == 1 and rec.name == record.ref:
                    sub = self.amount_total-record.amount
                    return sub
                if rec.cout_payment > 1 and rec.name == record.ref:
                    return self.amount_total-self.total_amount_pays
    # -------------------------------------------------
