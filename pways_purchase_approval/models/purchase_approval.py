...
class PurchaseApproval(models.Model):
    ...
    document_type = fields.Selection([('purchase', 'Purchase Order')], string='Document Type', required=True)
    ...
    def _check_duplicate_user_id(self):
        for approval in self:
            for user in approval.approval_line_ids.mapped("user_id"):
                line_ids = self.env['purchase.approval.lines'].search_count([('approval_id.document_type', '=', 'purchase'), ('user_id', '=', user.id)])
                if line_ids > 1:
